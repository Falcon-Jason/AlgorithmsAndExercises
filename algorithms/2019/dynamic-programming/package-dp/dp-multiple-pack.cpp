...
int dpMultiplePack(int *, int, int, int);
...
int main()
{
    ...
    int *limits = new int[count + 1];
    int *dpFunction = new int[maxWeight + 1];
    weights[0] = values[0] = limits[0] = dpFunction[0] = 0;

    readArray(weights + 1, count);
    readArray(values + 1, count);
    readArray(limits + 1, count);
    setArray(dpFunction + 1, 0, maxWeight);

    for (int i = 1; i <= count; i++)
    {
        maxValue = dpMultiplePack(dpFunction, weights[i], values[i], limits[i]);
    }
...
int dpMultiplePack(int *dpFunc, int weight, int value, int limit)
{
    for (int w = maxWeight; w >= weight; w--)
    {
        for (int k = 0; k * weight <= w && k <= limit; k++)
        {
